...
const reportCardBadge = {
    ...
    published: { label: 'Sudah Terbit', badge: 'bg-emerald-100 text-emerald-700', icon: 'check' },
};
...
<template>
    ...
    <AppLayout>
        ...
        <div class="space-y-5 pb-8">
            ...
            <!-- ── Welcome banner ─────────────────────────────────────────── -->
            <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-600 p-6 shadow-lg shadow-emerald-200">
            ...
            <template v-else>
                ...
                <div v-if="reportCardStatus"
                    ...
                    :class="reportCardStatus.status === 'published' ? 'border-emerald-200' : 'border-slate-100'">
                    <div class="flex size-11 shrink-0 items-center justify-center rounded-2xl"
                        :class="reportCardStatus.status === 'published' ? 'bg-emerald-50 ring-4 ring-emerald-100' : 'bg-slate-50 ring-4 ring-slate-100'">
                        <svg v-if="reportCardStatus.status === 'published'" class="size-5 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm font-bold"
                            :class="reportCardStatus.status === 'published' ? 'text-emerald-700' : 'text-slate-600'">
                    ...
                    <Link v-if="reportCardStatus.status === 'published'"
                        ...
                        class="shrink-0 rounded-xl bg-emerald-500 px-4 py-2 text-xs font-bold text-white shadow-sm shadow-emerald-100 transition hover:bg-emerald-600">
                        Lihat Raport
                    </Link>
                </div>
                ...
                <!-- ── Semua lunas ─────────────────────────────────────────── -->
                <div v-else
                    class="flex items-center gap-3.5 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 shadow-sm">
                    <div class="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-emerald-100">
                        <svg class="size-5 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <div>
                        <p class="text-sm font-bold text-emerald-800">Semua tagihan sudah lunas</p>
                        <p class="text-xs text-emerald-600">Tidak ada tagihan yang perlu dibayarkan saat ini.</p>
                    </div>
                </div>
