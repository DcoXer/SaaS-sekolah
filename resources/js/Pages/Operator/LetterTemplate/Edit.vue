...
const form = useForm({
    letter_type_id:         props.template.letter_type_id,
    name:                   props.template.name,
    content:                props.template.content,
    available_placeholders: props.template.available_placeholders ?? Object.keys(props.availablePlaceholders),
...
const submit = () => {
    form.put(route('operator.letter-templates.update', props.template.id), { onError: () => {} });
};
</script>

<template>
    <AppLayout>
        <Head title="Edit Template Surat" />
        ...
        <template #title>
            <div class="flex items-center gap-2 text-sm text-slate-500">
                ...
                <span>/</span>
                <span class="font-semibold text-slate-700">Edit Template</span>
            </div>
        </template>
        ...
        <div class="space-y-4">
            ...
            <!-- Heading -->
            <div>
                <h2 class="text-balance text-lg font-bold text-slate-900">Edit Template Surat</h2>
                <p class="text-pretty text-sm text-slate-500">
                    Ubah konten atau konfigurasi template surat.
                </p>
            </div>
                ...
                <form @submit.prevent="submit" class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="space-y-5 px-6 py-6">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <span class="text-xs text-slate-400">(template nonaktif tidak bisa dipakai untuk membuat surat baru)</span>
                        </div>
