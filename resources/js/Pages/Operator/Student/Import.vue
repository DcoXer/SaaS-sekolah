...
// If controller returned previewData, jump straight to step 2
...
const uploadPreview = () => {
    ...
    router.post(route('operator.students.import.preview'), formData, {
        forceFormData: true,
        onSuccess: () => {
            // previewData will be in page props — re-read
...
const confirmImport = () => {
    ...
    router.post(route('operator.students.import.confirm'), {
...
const existingSet = computed(() => new Set(preview.value?.existingNisns ?? []));
        ...
        <div class="space-y-6">
            ...
            <template v-if="step === 1">
                <div class="mx-auto max-w-lg space-y-5">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">Import Data Siswa</h2>
                        <p class="mt-1 text-sm text-slate-500">
                            Upload file Excel (.xlsx) atau CSV. Mendukung format template sendiri maupun format EMIS.
                        </p>
                    </div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <a
                                :href="route('operator.students.import.template')"
            ...
            <template v-else-if="step === 2 && preview">
                ...
                <div v-if="hasErrors" class="rounded-xl border border-red-200 bg-red-50 p-4">
                    ...
                    <ul class="space-y-1">
                        <li v-for="(err, i) in preview.errors" :key="i" class="text-xs text-red-600">
                            {{ err.message }}
                        </li>
                    </ul>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-3 text-sm">
                    ...
                    <span class="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2.5 py-0.5 text-xs font-semibold text-primary-700 ring-1 ring-primary-200">
                        Baru: {{ preview.rows.filter(r => !existingSet.has(r.nisn)).length }}
                    </span>
                    <span class="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-semibold text-amber-700 ring-1 ring-amber-200">
                        Update: {{ preview.rows.filter(r => existingSet.has(r.nisn)).length }}
                    </span>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-slate-100 text-xs">
                            <thead>
                                <tr class="bg-slate-50">
                                    ...
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">NISN</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Nama</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Kelas</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Kelamin</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Tempat Lahir</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Tgl Lahir</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">NIK</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Nama Ayah</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Nama Ibu</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Nama Wali</th>
                                    <th class="px-3 py-3 text-left font-semibold text-slate-500">Alamat</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100">
                                <tr v-for="(row, i) in preview.rows" :key="i" class="hover:bg-slate-50">
                                    ...
                                    <td class="px-3 py-2.5">
                                        <span v-if="existingSet.has(row.nisn)" class="inline-flex rounded-full bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-700 ring-1 ring-amber-200">Update</span>
                                    ...
                                    <td class="px-3 py-2.5 font-mono text-slate-700">{{ row.nisn || '—' }}</td>
                                    <td class="px-3 py-2.5 font-semibold text-slate-800">{{ row.name }}</td>
                                    <td class="px-3 py-2.5 text-slate-600">{{ row.grade ? 'Kelas ' + row.grade : '—' }}</td>
                                    <td class="px-3 py-2.5 text-slate-600">{{ genderLabel(row.gender) || '—' }}</td>
                                    <td class="px-3 py-2.5 text-slate-600">{{ row.birth_place || '—' }}</td>
                                    <td class="px-3 py-2.5 text-slate-600">{{ row.birth_date || '—' }}</td>
                                    <td class="px-3 py-2.5 font-mono text-slate-500">{{ row.nik || '—' }}</td>
                                    <td class="px-3 py-2.5 text-slate-600">{{ row.father_name || '—' }}</td>
                                    <td class="px-3 py-2.5 text-slate-600">{{ row.mother_name || '—' }}</td>
                                    <td class="px-3 py-2.5 text-slate-600">{{ row.guardian_name || '—' }}</td>
                                    <td class="px-3 py-2.5 text-slate-500">{{ row.address || '—' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
