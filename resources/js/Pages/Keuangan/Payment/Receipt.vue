        ...
        <div class="space-y-4">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-500">{{ payment_type?.name }} · {{ student?.name }}</p>
                </div>
                <a
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm print:border-0 print:shadow-none">
                    ...
                    <div class="px-4 py-5 sm:px-8">
                        ...
                        <dl class="space-y-2 text-sm">
                            <div class="flex justify-between gap-4">
                                ...
                                <dd class="font-semibold text-slate-800 text-right">{{ student?.name }}</dd>
                            </div>
                            <div class="flex justify-between gap-4">
                                <dt class="text-slate-500">NIS</dt>
                                <dd class="font-semibold text-slate-800">{{ student?.nis }}</dd>
                            </div>
                            <div v-if="invoice?.student?.classrooms?.[0]" class="flex justify-between gap-4">
                                <dt class="text-slate-500">Kelas</dt>
                                <dd class="font-semibold text-slate-800">{{ invoice.student.classrooms[0].name }}</dd>
                            </div>
                        </dl>
                    </div>
                    <div class="px-4 py-5 sm:px-8">
                ...
                <div class="px-4 py-5 sm:px-8">
                    ...
                    <div v-if="invoice?.payments?.length === 0" class="text-sm text-slate-400">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-slate-100 text-left">
                                ...
                                <th class="pb-2 text-xs font-semibold text-slate-500">Catatan</th>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            <tr v-for="pay in invoice.payments" :key="pay.id">
                                ...
                                <td class="py-2 text-slate-400">{{ pay.note ?? '-' }}</td>
                                <td class="py-2 text-right font-semibold text-emerald-600">{{ formatRupiah(pay.amount) }}</td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr class="border-t border-slate-200">
                                <td colspan="3" class="pt-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Total Terbayar</td>
